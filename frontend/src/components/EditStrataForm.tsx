...
import { Strata, StrataFormData, Server, IGroupServerConfig } from '@/types';
import { useStrataData } from '@/hooks/useStrataData';
import { useServerData } from '@/hooks/useServerData';
import { ServerToolConfig } from './ServerToolConfig';

interface EditStrataFormProps {
  strata: Strata;
  onEdit: () => void;
  onCancel: () => void;
}

const EditStrataForm = ({ strata, onEdit, onCancel }: EditStrataFormProps) => {
  const { t } = useTranslation();
  const { updateStrata } = useStrataData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateStrata(strata.id, {
        name: formData.name,
      ...
      if (!result || !result.success) {
        setError(result?.message || t('stratas.updateError') || 'Failed to update strata');
        setIsSubmitting(false);
        return;
      }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="flex justify-end space-x-3 p-6 pt-4 border-t border-[#ff6600]/30 flex-shrink-0">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? t('common.submitting') || 'Saving...' : t('common.save') || 'Save'}
            </button>
          </div>
        </form>
      </div>
